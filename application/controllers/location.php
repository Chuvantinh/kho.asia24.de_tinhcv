 ...
 */
class Location extends manager_base
{
            ...
            if($object){
                foreach($object->getWorksheetIterator() as $worksheet)
                {
                    ...
                    for($row=2; $row<=$highestRow; $row++)
                    {
                        ...
                        $this->data->add($data); // them data vao DB
                    }
                }
            }
...
// ở đây mình lưu file dưới dạng excel2007
    ...
    /*public function ajax_list_data($data = Array())
    {
        ...
        $all_location = $this->data->get_all_location();
        foreach ($all_location as $row){
            $group = substr($row['name'], 0,4);
            $this->data->update_location($row['id'],$group);
        }
