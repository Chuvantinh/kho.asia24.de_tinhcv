 ...
 */
class Voxy_transfer_out_kho extends manager_base
{
    ...
    {
        $this->name = Array(
            "class" => "voxy_transfer_out_kho",
            "view" => "voxy_transfer_out_kho",
            "model" => "m_voxy_transfer_out_kho",
            "object" => " Xuất Hàng Tại Kho"
        );
    }
...
//            }
    ...
    public function ajax_list_data_new($data = Array())
    {
        ...
        $totalItem  = -1;
        $record     = $this->m_voxy_transfer_out_kho->get_list_table($search_string, $where_condition, $limit, $post, $order, $totalItem);
        ...
        if (isset($data['call_api']) && $data['call_api']) {
            ...
            if ($totalItem < 0) {
                $totalItem = count($this->m_voxy_transfer_out_kho->get_list_table($search_string, $where_condition, 0, 0, $order));
            }
        }
    ...
    public function add_save($data = Array(), $data_return = Array(), $re_validate = true)
    {
        ...
        if ($insert_id) {
            ...
            if($data['status'] == 1){
                $data_return["msg"] = "Cập nhật Thành Công hàng về vào cơ sở dữ liệu. Bạn đã chọn XUẤT KHO";
                foreach ($list_product as $item){//add inventory
                    ...
                    if ($check_variant1_id == true) {
                        $this->m_voxy_package->update_minus_inventory1($item['sl_nhap'], $id);//in DB
                    }
                    if ($check_variant2_id == true) {
                        $this->m_voxy_package->update_minus_inventory2($item['sl_nhap'], $id);//in DB
                    }
                }
            }else{
                $data_return["msg"] = "Hàng về nhưng chưa được cập nhật vào cơ sở dữ liệu, Bạn phải chọn là XUẤT Kho";
            }
    ...
    public function search_pro()
    {
        ...
        if ($data['list_products'] == false) {
            ...
            $viewFile = '/voxy_transfer_out_kho/search_pro';
            $content = $this->load->view($this->path_theme_view . $viewFile, $data, true);
            $data_return["state"] = 1;
            $data_return["msg"] = "Ok";
            $data_return["html"] = $content;
            ...
            if ($object) {
                ...
                foreach ($object->getWorksheetIterator() as $worksheet) {
                    ...
                    for ($row = 2; $row <= $highestRow; $row++) {
                        ...
                        $variant_title = $worksheet->getCellByColumnAndRow(4, $row)->getValue();
                        $gianhapnew = $worksheet->getCellByColumnAndRow(5, $row)->getValue();
                        $thanhtien = $worksheet->getCellByColumnAndRow(6, $row)->getValue();
                        $cat_title = $worksheet->getCellByColumnAndRow(7, $row)->getValue();
                        $cat_id = $this->m_voxy_category->get_id_title($cat_title);
                        $product_id = $worksheet->getCellByColumnAndRow(8, $row)->getValue();
                        $variant_id = $worksheet->getCellByColumnAndRow(9, $row)->getValue();


                        $data_add = array(
                            'quantity' => $quantity,
                            'variant_title' => $variant_title,
                            'gianhapnew' => $gianhapnew,
                ...
                if ($data['list_products'] == false) {
                    ...
                    $viewFile = '/voxy_transfer_out_kho/list_products';
                    $content = $this->load->view($this->path_theme_view . $viewFile, $data, true);
                    $data_return["state"] = 1;
                    $data_return["msg"] = "Ok";
                    $data_return["html"] = $content;
