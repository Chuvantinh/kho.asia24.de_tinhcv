 ...
 */
class Admin_functions extends manager_base
{
    ...
    public function setting_class()
    {
        $this->name = Array(
            "class"     => "admin_functions",
            "view"      => "admin_functions",
            "model"     => "m_admin_functions",
            "object"    => "Permission"
        );
    }
    ...
    {
        $data['update_link'] = site_url($this->name['class'] . '/update_function');
        parent::manager($data);
    }
    ...
    /**
     * Ham lay danh sach cac controller khong can cho vao phan quyen
     * @return array
     *
     * @author chuvantinh1991@gmail.com
     */
    protected function get_list_not_controller()
    {
        $list_not_controller = Array();
        $this->load->model('m_sys_not_controller_config', 'm_sys_not_controller');

        $list_data = $this->m_sys_not_controller->get_list(Array('status' => 1));
        if($list_data){
            foreach ($list_data as $key => $value) {
                $list_not_controller[] = strtolower(trim($value->controller));
            }
        }

        return $list_not_controller;
    }

    /**
     * Ham lay danh sach cac Method khong can cho vao phan quyen
     * @return array
     *
     * @author chuvantinh1991@gmail.com
     */
    protected function get_list_not_method()
    {
        $list_not_method = Array();
        $this->load->model('m_sys_not_method_config', 'm_sys_not_method');

        $list_data = $this->m_sys_not_method->get_list(Array('status' => 1));
        if($list_data){
            foreach ($list_data as $key => $value) {
                $list_not_method[] = strtolower(trim($value->method));
            }
        }

        return $list_not_method;
    }

    /**
     * Ham cap nhat danh sach Function vao bang phan quyen
     *
     * @author chuvantinh1991@gmail.com
     */
    public function update_function()
    {
        ...
        $controller_list        = $this->controllerlist->getControllers();
        $list_not_controller    = $this->get_list_not_controller();
        $list_not_method        = $this->get_list_not_method();
        ...
        foreach($controller_list as $controller_name => $arr_method) {
            foreach($arr_method as $method){
                ...
                if(in_array($method, $list_not_method) || in_array($controller_name, $list_not_controller) || $this->data->check_exist_by_controller_action($controller_name, $method)) {
                    continue;
                }
                $arr_data_method[] = Array(
                    'name' 			=> $name_function,
