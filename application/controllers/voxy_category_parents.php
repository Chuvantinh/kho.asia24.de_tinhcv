 ...
 */
class Voxy_category_parents extends manager_base
{
    ...
    {
        $this->name = Array(
            "class" => "voxy_category_parents",
            "view" => "voxy_category_parents",
            "model" => "m_voxy_category_parents",
            "object" => "Danh Mục Cha"
        );
    }
    ...
    public function add_save($data = Array(), $data_return = Array(), $re_validate = true)
    {
        ...
        if ($insert_id) {
            ...
            $data_return["msg"] = "Thêm bản ghi thành công vào database";
            ...
            $data_return["msg"] = "Thêm bản ghi thất bại vao database, vui lòng thử lại sau";
            echo json_encode($data_return);
            return FALSE;
        }
    ...
    public function edit_save($id = 0, $data = Array(), $data_return = Array(), $re_validate = true)
    {
        ...
        $this->load->model('m_voxy_category_parents', 'category');
        // Check titel category
        $exist_status = $this->data->check_exist_title($data['title']);
        if ($exist_status === NULL) {
            $data_return["state"] = 0;
