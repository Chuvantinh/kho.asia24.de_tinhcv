 ...
 */
class Voxy_kiemkho_theongay extends manager_base
{
    ...
    {
        $this->name = Array(
            "class" => "voxy_kiemkho_theongay",
            "view" => "voxy_kiemkho_theongay",
            "model" => "m_voxy_kiemkho_theongay",
            "object" => " Kiểm Kho Theo Ngày"
        );
    }
...
//            }
    ...
    public function search_pro()
    {
        ...
        if ($data['list_products'] == false) {
            ...
            $viewFile = '/voxy_kiemkho_theongay/search_pro';
            $content = $this->load->view($this->path_theme_view . $viewFile, $data, true);
            $data_return["state"] = 1;
            $data_return["msg"] = "Ok";
            $data_return["html"] = $content;
...
// ở đây mình lưu file dưới dạng excel2007
    ...
    public function kiemkho_theongay(){
        $this->load->model('m_voxy_kiemkho_theongay');
        $this->load->model('m_voxy_package');
        $data_add_database = array();

        $date = date("Y-m-d");
        $data_add_database['date_save'] = $date;
        $data_add_database['status'] = 1;

        $all_product_inkho =$this->m_voxy_package->get_nur_product_inkho();
        $i = 0;
        $array_end = array();
        $total_price = 0;
        foreach ($all_product_inkho as $item){
            if($item['variant1_id'] != ""){
                $i++;
                $array_end[$i]['v_id'] = $item['variant1_id'];
                $array_end[$i]['sl'] = $item['inventory_quantity1'];
                $total_price += (double)$item['inventory_quantity1'] * (double)$item['gia_mua_le'];
            }
            if($item['variant2_id'] != ""){
                $i++;
                $array_end[$i]['v_id'] = $item['variant2_id'];
                $array_end[$i]['sl'] = $item['inventory_quantity2'];
                $total_price += (double)$item['inventory_quantity2'] * (double)$item['gia_mua_si'];
            }
        }
        $data_add_database['total_price'] = $total_price;
        $data_add_database['product_variants'] = json_encode($array_end);

        if($data_add_database){

            $check = $this->m_voxy_kiemkho_theongay->check_kiemkho_theongay($date);
            if($check != false){
                //update
                $id = $this->m_voxy_kiemkho_theongay->check_kiemkho_theongay($date);
