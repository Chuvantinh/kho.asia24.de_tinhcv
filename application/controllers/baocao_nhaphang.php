 ...
 */
class Baocao_nhaphang extends manager_base
{
    ...
    {
        $this->name = Array(
            "class" => "baocao_nhaphang",
            "view" => "baocao_nhaphang",
            "model" => "m_baocao_nhaphang",
            "object" => " Báo Cáo Nhập Hàng"
        );
    }
    ...
    protected function get_search_condition($data = array())
    {
        ...
        $where_data = array();
        $like_data = array();
        $list_field = array('ngay_dat_hang','ngay_giao_hang','vendor', 'status');
...
//            }
    ...
    {
        $this->load->model('m_baocao_nhaphang');
        ...
        $totalItem  = -1;
        $record     = $this->m_baocao_nhaphang->get_list_table($search_string, $where_condition, $limit, $post, $order, $totalItem);
        ...
        if (isset($data['call_api']) && $data['call_api']) {
            ...
            if ($totalItem < 0) {
                $totalItem = count($this->m_baocao_nhaphang->get_list_table($search_string, $where_condition, 0, 0, $order));
            }
        }
