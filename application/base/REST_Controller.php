...
abstract class REST_Controller extends CI_Controller {
    ...
    protected $_client_domain   = '';       // Domain cua he thong gi API
    protected $_year_month      = '';
    protected $_rest_logs_table = '';       // ten bang log rest se luu
    protected $_maintenance     = FALSE;    // trang thai bao tri he thong
    protected $_weight_id       = 0;        // Weight id
    protected $_weight          = 0;        // Trong so fake User ID
    protected $_connect         = '';       // Chuoi ket noi fake Email
    ...
    public function __construct() {
        ...
        $this->request      = new stdClass(); // let's learn about the request
        $this->response     = new stdClass();
        $this->_zlib_oc     = @ini_get('zlib.output_compression');
        $this->_year_month  = date('Ym', time());

        // Lets grab the config and get ready to party
        $this->load->config('rest');
        $this->_rest_logs_table = config_item('rest_logs_table') . '_' . $this->_year_month;
        ...
        $this->request->ssl     = $this->_detect_ssl();     // Is it over SSL?
        $this->request->method  = $this->_detect_method();  // How is this request being made? POST, DELETE, GET, PUT?
        ...
        $this->request->body    = NULL;// Some Methods cant have a body
        ...
        $this->response->lang   = $this->_detect_lang(); // Which format should the data be returned in?
        ...
        if ($this->auth_override !== TRUE) {
            ...
            } else if (strtolower($this->config->item('rest_auth')) == 'digest') {
                $this->_prepare_digest_auth();
            } else if ($this->config->item('rest_ip_whitelist_enabled')) {
        ...
        } else if (property_exists($this, "db")) { // Use whatever database is in use (isset returns false)
        ...
        if (!$this->input->is_ajax_request() AND config_item('rest_ajax_only')) {
            $response = $this->_error(NULL, 'REST_AJAX_ONLY', 'Only AJAX requests are accepted.');
            $this->response($response, 505);
        }

        // Kiem tra su ton tai cua domain he thong
        $this->load->model('m_sys_system_config', 'sys_system');
        $status_system_exists = $this->sys_system->get_one(array('m.domain' => $this->_client_domain), 'object');
        if($status_system_exists && is_object($status_system_exists)){
            $this->_weight_id   = isset($status_system_exists->weight_id)   ? $status_system_exists->weight_id  : '0';
            $this->_weight      = isset($status_system_exists->w_weight)    ? $status_system_exists->w_weight   : '0';
            $this->_connect     = isset($status_system_exists->w_connect)   ? $status_system_exists->w_connect  : '';
        } else {
            $response = $this->_error(NULL, 'SYSTEM_NOT_HAVE_ACCESS', "{$this->_client_domain} không có quyền truy cập vào hệ thống API !");
            $this->response($response, 200);
        }

        // kiem tra gia tri bao tri he thong
        $this->load->model('m_sys_voxy_maintenance', 'maintenance');
        $this->_maintenance = $this->maintenance->get_status_maintenance($this->_client_domain);
        if($this->_maintenance){
            $response = $this->_error(NULL, 'MAINTENANCE', 'Hệ thống đang bảo trì, bạn vui lòng quay trở lại sau !');
            $this->response($response, 200);
        }
    }
    ...
    public function _remap($object_called, $arguments) {
        ...
        if (config_item('force_https') AND !$this->_detect_ssl()) {
            $response = $this->_error(NULL, 'UNSUPPORTED_PROTOCOL', 'Unsupported protocol.');
        ...
        if (config_item('rest_enable_keys') AND $use_key) {
            if ($this->_allow === FALSE) {
                ...
                $response = $this->_error(NULL, 'INVALID_API_KEY', 'Invalid API Key.');
                $this->response($response, 403);
            }
            // Check to see if they can access the controller
            if (!$this->_check_access()) {
                $response = $this->_error(NULL, 'API_KEY_NOT_HAVE_ACCESS', 'This API key does not have access to the requested controller.');
        ...
        if (!method_exists($this, $controller_method)) {
            $response = $this->_error(NULL, 'UNKNOWN_METHOD', 'Unknown method.');
        ...
        if (config_item('rest_enable_keys') AND !empty($this->rest->key)) {
            ...
            if (config_item('rest_enable_limits') AND !$this->_check_limit($controller_method)) {
                $response = $this->_error(NULL, 'API_KEY_HAS_REACHED_LIMIT', 'This API key has reached the hourly limit for this method.');
                $this->response($response, 401);
            }
            ...
            if(!$authorized){
                $response = $this->_error(NULL, 'API_NOT_HAVE_ENOUGH_PERMISSIONS', 'This API key does not have enough permissions.');
                $this->response($response, 401);
            }

        } else if (config_item('rest_enable_logging') AND $log_method) { // No key stuff, but record that stuff is happening
        ...
        try {
            ...
            $error              = new stdClass();
            $error->status      = FALSE;
            $error->status_code = 'EXCEPTION';
            $error->msg         = 'Method does not exist.';
            $error->data        = NULL;
            $error->error       = array (
    ...
    public function response($data = null, $http_code = null) {
        ...
        if ($data === NULL && $http_code === null) {
        ...
        } else if ($data === NULL && is_numeric($http_code)) { // If data is NULL but http code provided, keep the output empty
            $output = NULL;
            ...
            } else if (method_exists($this->format, 'to_' . $this->response->format)) { // If the format method exists, call and return the output in that format
                // Set the correct format header
                header('Content-Type: ' . $this->_supported_formats[$this->response->format]);
    ...
    protected function _detect_output_format() {
        ...
        if (preg_match($pattern, $this->uri->uri_string(), $matches)) {
            return $matches[1];
        } else if ($this->_get_args AND !is_array(end($this->_get_args)) AND preg_match($pattern, end($this->_get_args), $matches)) {  // Check if a file extension is used
                ...
                if (strpos($this->input->server('HTTP_ACCEPT'), $format) !== FALSE) {
                    ...
                    if ($format != 'html' AND $format != 'xml') {
                        ...
                        if ($format == 'html' AND strpos($this->input->server('HTTP_ACCEPT'), 'xml') === FALSE) {
                            return $format;
                        } else if ($format == 'xml' AND strpos($this->input->server('HTTP_ACCEPT'), 'html') === FALSE) { // If it is truly XML, it wont want any HTML
                            return $format;
                        }
                    }
                }
    ...
    protected function _detect_api_key() {
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name))) {
            ...
            $this->_api_user = $row; // chua hieu dung de lam gi

            /*
             * If "is private key" is enabled, compare the ip address with the list
             * of valid ip addresses stored in the database.
             */
            if (isset($row->is_private_key) && $row->is_private_key) {
                $break_private = FALSE;
                // Check for a list of valid domain
                if(isset($row->domain_name)){
                    // multiple ip addresses must be separated using a comma, explode and loop
                    $list_domain_name  = explode(";", $row->domain_name);
                    foreach ($list_domain_name as $domain_name) {
                        if ($this->_client_domain == trim($domain_name)) {
                ...
                } else if (isset($row->ip_addresses)) {
                    // multiple ip addresses must be separated using a comma, explode and loop
    ...
    protected function _log_request($authorized = FALSE) {
        $this->_check_rest_logs_table_exist();
        $status = $this->rest->db->insert($this->_rest_logs_table, array(
            'uri'           => $this->uri->uri_string(),
            'method'        => $this->request->method,
            'params'        => $this->_args ? (config_item('rest_logs_json_params') ? json_encode($this->_args) : serialize($this->_args)) : null,
            'api_key'       => isset($this->rest->key) ? $this->rest->key : '',
            'domain_name'   => $this->_client_domain,
            'ip_address'    => $this->input->ip_address(),
            'created_at'    => function_exists('now') ? now() : time(),
    ...
    /**
     * Kiem tra ham tra va tao bang rest logs
     * @return bool
     *
     * @author chuvantinh1991@gmail.com
     */
    protected function _check_rest_logs_table_exist(){
        if($this->rest->db->table_exists($this->_rest_logs_table)){
            return true;
        } else {
            return $this->_create_table_rest_logs($this->_rest_logs_table);
        }
    }

    /**
     * Ham tao bang rest log theo thang
     * @param $table_name
     * @return mixed
     *
     * @author chuvantinh1991@gmail.com
     */
    protected function _create_table_rest_logs($table_name) {
        $fields = array(
            'id int(11) NOT NULL AUTO_INCREMENT',
            'uri varchar(255) COLLATE utf8_general_ci NULL DEFAULT NULL',
            'method varchar(255) COLLATE utf8_general_ci NULL DEFAULT NULL',
            'params text COLLATE utf8_general_ci NULL DEFAULT NULL',
            'api_key varchar(50) COLLATE utf8_general_ci NULL DEFAULT NULL',
            'domain_name varchar(255) COLLATE utf8_general_ci NULL DEFAULT NULL',
            'ip_address varchar(255) COLLATE utf8_general_ci NULL DEFAULT NULL',
            'http_code varchar(50) COLLATE utf8_general_ci NULL DEFAULT NULL',
            'response_data text COLLATE utf8_general_ci NULL DEFAULT NULL',
    ...
    protected function _prepare_basic_auth() {
        ...
        if ($this->input->server('PHP_AUTH_USER')) {
            ...
            $arr_passwd = explode('/', $password);
            if(count($arr_passwd) > 1){
                $this->load->library('ThuyVu_lib');
                $this->_client_domain = $this->thuyvu_lib->cut_short_domain(substr($password, strlen($arr_passwd[0]) + 1, strlen($password)-1));
                $password = $arr_passwd[0];
            }

        } else if ($this->input->server('HTTP_AUTHENTICATION')) { // most other servers
    ...
    protected function _check_blacklist_auth() {
        ...
        if (!in_array($this->input->ip_address(), $blacklist)) {
            $response = $this->_error(NULL, 'IP_DENIED', 'IP Denied');
            $this->response($response, 401);
        }
    }
    ...
    protected function _check_whitelist_auth() {
        ...
        if (!in_array($this->input->ip_address(), $whitelist)) {
            $response = $this->_error(NULL, 'NOT_AUTHORIZED', 'Not authorized');
            $this->response($response, 401);
        }
    }
    ...
    protected function _log_access_time() {
        ...
        $payload['response_data']   = json_encode($this->_response_data);
        $payload['http_code']       = $this->_http_code;
        if (isset($this->rest->db)) {
            return $this->rest->db->update($this->_rest_logs_table, $payload, array('id' => $this->_insert_id));
        } else {
            return false;
        }
    }
    ...
    protected function _check_access() {
        ...
        $this->rest->db->select();
        $this->rest->db->from(config_item('rest_access_table') . " AS raccess");
        $this->rest->db->join(config_item('rest_keys_table') . " AS rkey", "rkey.id = raccess.key_id");
        $this->rest->db->where('rkey.' . config_item('rest_key_column'), $this->rest->key);
        $this->rest->db->where('raccess.controller', $this->router->fetch_class());
        $this->rest->db->where('rkey.status', 1);
        $this->rest->db->where('raccess.status', 1);
        $query = $this->rest->db->get();
        if ($query->num_rows > 0) {
            $result = $query->result();
            foreach ($result as $item) {
                $listMethod = explode(";", $item->method);
                if (!isset($item->method) || strlen($item->method) == 0) {
                    $item->method = NULL;
                }
                if ($item->method === NULL || in_array($this->router->fetch_method(), $listMethod)) {
                    return TRUE;
                }
            }
        }
    ...
    public function get_clientname() {
        $username = $this->_auth_username;
        return $username . '(' . $this->input->ip_address() . ')';
    }

    /**
     * Lay domain cua thang goi API (duoc truyen theo passwd)
     * @return mixed
     *
     * @author chuvantinh1991@gmail.com
     */
    public function get_client_domain(){
        return $this->_client_domain;
    }

    /**
     * Ham tra ve thong tin co ban ma Client API goi toi
     * @return array
     *
     * @author chuvantinh1991@gmail.com
     */
    public function get_client_info(){
        return array(
            ...
            'client_domain' => $this->_client_domain,
            'auth_username' => $this->_auth_username,
            'weight_id'     => $this->_weight_id,
            'weight'        => $this->_weight,
