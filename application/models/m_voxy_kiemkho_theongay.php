 ...
 */
class M_voxy_kiemkho_theongay extends data_base
{
    var $vendor = array(
        '0' => '- Lựa chọn giá trị -',
        'LIL' => 'LIL',
    ...
    {
        $this->_table_name = 'voxy_kiemkho_theongay';
        $this->_key_name = 'id';
...
//dung nhung function duoi day
    public function get_products_selected($id)
    {
        ...
        $this->db->from('voxy_kiemkho_theongay');
        $this->db->where("id", $id);
        $query = $this->db->get();
        if ($query->result_array()) {
            foreach ($query->result() as $row) {
    ...
    public function check_kiemkho_theongay($date){
        $this->db->select('id');
        $this->db->from('voxy_kiemkho_theongay');
        $this->db->like('date_save',$date);
        $query = $this->db->get();
        //var_dump($this->db->last_query());die;
        if ($query->result_array()) {
            foreach ($query->result_array() as $item){
    ...
    public function get_product_variants_kitruoc_date_new($date){
        $this->db->select('product_variants');
        $this->db->from('voxy_kiemkho_theongay');
        $this->db->like('date_save',$date);
        $query = $this->db->get();
