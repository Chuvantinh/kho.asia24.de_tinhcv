 ...
 */
class M_baocao_nhaphang extends data_base
{
    var $vendor = array(
        '0' => '- Lựa chọn giá trị -',
        'LIL' => 'LIL',
    ...
    {
        $this->_table_name = 'voxy_transfer';
        $this->_key_name = 'id';
        ...
        $this->_schema = Array(
            ...
            'tracking_nummer','tags','date_save','mswt',
    ...
    public function setting_select()
    {
        ...
        $this->db->where_in('status',array(1,2,5));
...
//dung nhung function duoi day
    ...
    public function get_infor_theo_ngay_hangve($id, $ngay_giao_hang,$ngay_dat_hang)
    {
        $this->db->select('product_variants');
        $this->db->from('voxy_transfer');
        if(isset($id['list_id']) && $id['list_id'] != ""){
    ...
    public function get_infor_theo_ngay_hanghong($id, $ngay_giao_hang,$ngay_dat_hang)
    {
        $this->db->select('product_variants');
        $this->db->from('voxy_transfer');
        if(isset($id['list_id']) && $id['list_id'] != ""){
    ...
    public function get_list_table($search_text = "", $whereCondition = NULL, $limit = 0, $post = 0, $order = NULL, &$total = 0)
    {
        ...
        if (is_array($whereCondition)) {
            ...
            if(isset($whereCondition["custom_where"]['m.status'])){
                $stauts = $whereCondition["custom_where"]['m.status'];
            }
            ...
            if(isset($stauts) && $stauts != null){
                $this->db->where("m.status",$stauts);
            }
