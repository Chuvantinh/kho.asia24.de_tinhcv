 ...
 */
class M_voxy_hangtralai_nhacc extends data_base
{
    ...
    {
        $this->_table_name = 'voxy_hangtralai_nhacc';
        $this->_key_name = 'id';
        ...
        $this->_schema = Array(
            ...
            'created_at', 'created_by', 'updated_at', 'updated_by','total_price'
        );
        $this->_rule = Array(
            'id' => array(
                'type' => 'hidden'
    ...
    public function get_list_table($search_text = "", $whereCondition = NULL, $limit = 0, $post = 0, $order = NULL, &$total = 0)
    {
        ...
        if (is_array($whereCondition)) {
            ...
            if(isset($whereCondition["custom_where"]['m.status'])){
                $status = $whereCondition["custom_where"]['m.status'];
            }
