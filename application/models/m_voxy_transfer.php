 ...
 */
class M_voxy_transfer extends data_base
{
    var $vendor = array(
        '0' => '- Lựa chọn giá trị -',
        'LIL' => 'LIL',
...
//dung nhung function duoi day
    ...
    public function get_nhaphang_vs_product($date, $date_end)
    {
        $this->load->model("m_voxy_nha_cung_cap");
        $this->db->select('vendor');
        $this->db->select('total_price');//theo brutto het roi
        ...
        $this->db->where("status",1);//da nhap kho
