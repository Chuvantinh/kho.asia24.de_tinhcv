...
class Rest_Client {
    ...
    protected $rest             = NULL; // rest_client
    ...
    function __construct($config = array()) {
        ...
        $this->_ci->load->config('rest_client');

        $this->rest = new StdClass();

        // Load DB if its enabled
        if (config_item('rest_client_db_group') AND config_item('rest_client_enable_logging')) {
            $this->rest->db = $this->_ci->load->database(config_item('rest_client_db_group'), TRUE);
    ...
    protected function _call($method, $uri, $params = array(), $format = NULL) {
        ...
        if (config_item('rest_client_db_group') AND config_item('rest_client_enable_logging')) {
            $this->_log_request($method, $uri, $params);
        }
        ...
        $this->_response = $this->_ci->curl->execute();
        ...
        if (config_item('rest_client_db_group') AND config_item('rest_client_enable_logging')) {
            $this->_log_request_time();
        }

        // Format and return
        return $this->_format_response($this->_response);
    }
     ...
     * @author chuvantinh1991@gmail.com
     */
    protected function _log_request($method, $uri, $param) {
        $status = false;
        $data = array(
            'uri'           => $uri,
            'method'        => $method,
        ...
        if (isset($this->rest->db) && $this->rest->db->table_exists(config_item('rest_client_logs_table'))) {
            $status             = $this->rest->db->insert(config_item('rest_client_logs_table'), $data);
            $this->_insert_id   = $this->rest->db->insert_id();
        }

        $data['params'] = $param;
        $string_logs = 'Startlog.' . $this->_insert_id . ' - ' . date('Y-m-d H:i:s', time()) . ' - ' . json_encode($data) . "\n";
        write_file(APPPATH . 'logs/log-rest-client-' . date('Y-m-d') . '.php', $string_logs, FOPEN_WRITE_CREATE);

        return $status;
    }

    /**
     * Ham update rtime khi goi api
     * @return bool
     *
     * @author chuvantinh1991@gmail.com
     */
    protected function _log_request_time() {
        $status                 = false;
        $payload['rtime']       = $this->_end_rtime - $this->_start_rtime;
        $payload['response']    = (is_string($this->_response) ? $this->_response : json_encode($this->_response));

        if (isset($this->rest->db) && $this->rest->db->table_exists(config_item('rest_client_logs_table'))) {
            $status = $this->rest->db->update(config_item('rest_client_logs_table'), $payload, array('id' => $this->_insert_id));
        }

        $string_logs = 'Endlog.' . $this->_insert_id . ' - ' . date('Y-m-d H:i:s', time()) . ' - ' . json_encode($payload) . "\n";
        write_file(APPPATH . 'logs/log-rest-client-' . date('Y-m-d') . '.php', $string_logs, FOPEN_WRITE_CREATE);
