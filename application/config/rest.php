...
|
*/
$config['rest_keys_table'] = 'admin_rest_keys';
...
|	FALSE
	...
	CREATE TABLE `admin_rest_keys` (
      `id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'id ban ghi',
      `key` varchar(41) NOT NULL COMMENT 'key - tu sinh',
      `level` int(2) NOT NULL COMMENT 'level truy cap',
      `ignore_limits` tinyint(1) NOT NULL DEFAULT '0' COMMENT 'gioi han so luong truy cap',
      `is_private_key` tinyint(1) NOT NULL DEFAULT '0' COMMENT 'private key',
      `domain_name` text COMMENT 'gioi han domain truy cap',
      `ip_addresses` text COMMENT 'gioi han dia chi ip truy cap',
      `status` tinyint(1) DEFAULT NULL COMMENT '1 active, 0 deactive',
      `created_at` int(11) DEFAULT NULL COMMENT 'thoi gian tao key',
      `created_by` int(11) DEFAULT NULL COMMENT 'id nguoi tao key',
      `updated_at` timestamp NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT 'thoi gian update',
      `updated_by` int(11) DEFAULT NULL COMMENT 'id nguoi update',
      PRIMARY KEY (`id`)
...
|
*/
$config['rest_logs_table'] = 'admin_rest_logs';
...
|
*/
$config['rest_access_table'] = 'admin_rest_access';
...
|	FALSE
|
    CREATE TABLE `admin_rest_access` (
      `id` int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT 'id ban ghi',
      `key_id` int(11) NOT NULL,
      `controller` varchar(50) NOT NULL DEFAULT '',
      `method` text,
      `status` tinyint(2) DEFAULT '1' COMMENT '1 active, 0 deactive',
      `created_at` int(11) DEFAULT NULL COMMENT 'thoi gian tao',
      `created_by` int(11) DEFAULT NULL COMMENT 'nguoi tao',
      `updated_at` timestamp NULL DEFAULT NULL,
      `updated_by` int(11) DEFAULT NULL COMMENT 'id nguoi chinh sua',
      PRIMARY KEY (`id`),
      KEY `key_id` (`key_id`),
      CONSTRAINT `admin_rest_access_ibfk_1` FOREIGN KEY (`key_id`) REFERENCES `admin_rest_keys` (`id`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8;
|
*/
...
|
*/
$config['rest_limits_table'] = 'admin_rest_limits';
...
|	FALSE
|
	CREATE TABLE `admin_rest_limits` (
	  `id` int(11) NOT NULL AUTO_INCREMENT,
	  `uri` varchar(255) NOT NULL,
...
| REST USERS
|--------------------------------------------------------------------------
    CREATE TABLE `admin_rest_users` (
        `id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'id ban ghi',
        `user_name` varchar(50) CHARACTER SET utf8 NOT NULL COMMENT 'user name dang nhap',
        `password` varchar(50) CHARACTER SET utf8 NOT NULL COMMENT 'password dang nhap',
        `password_show` varchar(50) DEFAULT NULL COMMENT 'password show',
        `salt` varchar(255) CHARACTER SET utf8 DEFAULT NULL,
        `check_sum` varchar(255) CHARACTER SET utf8 DEFAULT NULL,
        `status` tinyint(2) NOT NULL DEFAULT '1' COMMENT '1: active, 0 deactive',
        `created_at` int(11) DEFAULT NULL COMMENT 'thoi gian tao ban ghi',
        `created_by` int(11) DEFAULT NULL COMMENT 'id nguoi tao',
        `updated_at` timestamp NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT 'thoi gian cap nhat ban ghi',
        `updated_by` int(11) DEFAULT NULL COMMENT 'id nguoi cap nhat',
        PRIMARY KEY (`id`)
