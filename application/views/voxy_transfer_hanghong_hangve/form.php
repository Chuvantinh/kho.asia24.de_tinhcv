                    ...
                    if ($input->rule['type'] == "products") { ?>
                                <div class="products">
                                    ...
                                    <div class="wrapper_information" style="font-weight:bold; text-align: center;    height: 20px;">
                                        ...
                                        <div class="title" style="width: 43%;float: left;">Tên</div>
                                        <div class="sl_nhap" style="width: 10%;float: left;">SL</div>
                                        <div class="variant_title" style="width: 10%;float: left;">Đơn vị</div>
                                        <div class="variant_title" style="width: 10%;float: left;">Đơn hàng</div>
                                        <div class="variant_title" style="width: 10%;float: left;">Giá trị</div>
                                    ...
                                    <div class="list_products_transfer">
                                        ...
                                        foreach (json_decode($products_history) as $item) {
                                            ...
                                            <div  class="infomation <?php echo $class; ?>" style="padding: 5px 0; display: block;width: 100%;float: left;">
                                                ...
                                                <div style='width: 43%;height: auto;float: left; text-align: center;margin-right: 10px;' class='title'><?php echo $item['title'];?></div>
                                                <div style='width: 10%;height: auto;float: left'>
                                                    <input type='text' name='quantity[<?php echo $item['variant_id'];?>]'
                                                           ...
                                                           class='quantity'
                                                           style='min-width:50px;width: 50px;text-align: center;' <?php echo $_str_readonly; ?> >
                                                </div>
                                                <div style='width: 10%;height: auto;float: left;text-align: left !important;' class='variant_title'><?php echo $item['variant_title'];?></div>

                                                <div style='width: 10%;height: auto;float: left'>
                                                    <input type='text' name='donhang[<?php echo $item['variant_id'];?>]'
                                                           value="<?php echo isset($item['donhang'])?$item['donhang']:0;?>"
                                                           class='donhang'
                                                           style='min-width:50px;width: 50px;text-align: center;' <?php echo $_str_readonly; ?> >
                                                </div>

                                                <div style='width: 10%;height: auto;float: left;text-align: center !important;' class='giatri-<?php echo $item['variant_id'];?>'>
                                                    <input type='text' name='input_giatri[<?php echo $item['variant_id'];?>]'
                                                           value="<?php echo isset($item['thanhtien'])?$item['thanhtien']:0;?>"
                                                           class='input_giatri'
                                                           style='min-width:50px;width: 50px;text-align: center;' <?php echo $_str_readonly; ?> >
                                                </div>
...
<script type="text/javascript" charset="utf-8">
    $( document ).ready(function() {
        ...
        $("body").on("change", ".quantity", function(e) {
            var value = $(this).val();
            $(".donhang").attr('data-soluong',value);
        });

        $("body").on("change", ".donhang", function(e) {
                var textbox_class = $(this);
                var donhang_nummer = $(this).val();
                var variant_id = $(this).attr('data-variant-id');
                var soluong = $(this).attr('data-soluong');
            $.ajax({
                url: "<?php echo base_url(); ?>voxy_transfer_hanghong_hangve/get_price_for_product",
                async: false,
                type: "POST",
                dataType: "json",
                data : {
                    donhang_nummer: donhang_nummer,
                    variant_id: variant_id
                },
                success :function (data) {
                    if(data.state === 1){
                        var tongtien = soluong * data.price;
                        $('.giatri-'+variant_id + ' .input_giatri').attr('value',tongtien);
                    }else{
                        $('.giatri-'+variant_id + ' .input_giatri').attr('value',0);
                    }
                },
                error: function () {
                    console.log("loi ajax get id");
        ...
        $(".search_pro").on("change",function(e){
            ...
            if(request.length > 2){
                $.ajax({
                    url: "<?php echo base_url(); ?>voxy_transfer_hanghong_hangve/search_pro",
                    async: false,
                    type: "POST",
                    dataType: "json",
        ...
        //        products.push
        //        ({
        //            variant_id: $(this).find('.variant_id').text(),
        //            cat_id: $(this).find('.cat_id').text(),
        //            sl_kho: $(this).find('.sl_kho').text(),
        //            product_id: $(this).find('.product_id'),//4 cai an
        //            sku: $(this).find('.sku').text(),
        //            title: $(this).find('.title').text(),
        //            quantity: $(this).find('.quantity').val(),
        //            variant_title: $(this).find('.variant_title').text(),
        ...
        //    $.ajax({
        //        url: "<?php //echo base_url(); ?>//voxy_transfer/add_save",
        //        async: false,
        //        type: "POST",
        //        dataType: "json",
