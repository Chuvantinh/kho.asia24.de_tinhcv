...
Lọc: <input type="text" name="q" class="e_search_table" value="<?php echo $search_string; ?>" style="width: 80%;" autocomplete="off" />
        </div>
    </div>
    <div class="clear"></div>
</div>
...
<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered table-hover">
    ...
    <?php if (sizeof($record)) { ?>
        ...
        foreach ($record as $item) {
            ...
            ?>
            <tr class="gradeX <?php echo $class; ?> <?php echo $item->status; ?>" data-id="<?php echo $item->$key_name; ?>">
                <?php
                foreach ($colum as $key => $value) { ?>
                    <td for_key="<?php echo $key; ?>">
                        ...
                        if ($key == 'm.status') {
                            ...
                            if ($item->status == "black") {
                                $item->status = "xong";
                            } elseif ($item->status == "blue") {
                                $item->status = "chưa xong";
                            } else {
                                $item->status = "huỷ bỏ ";
                            }
                            echo $item->status;
