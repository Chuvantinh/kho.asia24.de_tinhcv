                    ...
                    if ($input->rule['type'] == "products") { ?>
                                <div class="products">
                                    ...
                                    <div class="wrapper_information" style="font-weight:bold; text-align: center;    height: 20px;">
                                        ...
                                        <div class="gianhap" style="width: 10%;float: left;text-align: right">Giá</div>
                ...
                <?php if ($input->name != "note" && $input->name != "products") { ?>
                    ...
                    <?php }elseif ($input->name == "datetime") { ?>
                        <div class="datetime">
                            ...
                            <?php } else { ?>
                                <input  autocomplete="off" class="text-right" name="<?php echo $input->name; ?>" <?php echo $input->string_rule; ?> id="i_<?php echo $input->name; ?>" />
                            <?php } ?>
                        </div>
                            ...
                            <?php if ($input->rule['type'] == 'select') { ?>
                                <select class="select2" name="<?php echo $input->name; ?>" id="i_<?php echo $input->name; ?>" <?php echo $input->string_rule; ?>>
                                    <?php foreach ($input->option as $option) { ?>
                                        <option value="<?php echo $option->value; ?>"><?php echo $option->display; ?></option>
                                    <?php } ?>
                                    <?php if($input->name == "laixe"){ ?>
                                        <option value="vanglai">Vang Lai</option>
                                    <?php } ?>
                                </select>
                            <?php } else { ?>
                                <input  class="text-right" name="<?php echo $input->name; ?>" <?php echo $input->string_rule; ?> id="i_<?php echo $input->name; ?>" />
...
<script type="text/javascript" charset="utf-8">
    $( document ).ready(function() {
            ...
            $.ajax({
                ...
                success :function (data) {
                    if(data.state === 1){
                        ...
                        $('.info_products').html('<h3>Không có sản phẩm</h3>');
                    }
                },
                error: function () {
                    console.log("loi ajax get id");
        ...
        $(".search_pro").on("change",function(e){
            ...
            if(request.length > 2){
                $.ajax({
                    url: "<?php echo base_url(); ?>voxy_hangtralai_nhacc/search_pro",
                    async: false,
                    type: "POST",
                    dataType: "json",
                    ...
                    success: function (data) {
                        if (data.state === 1) {
                            ...
                            search.css("display","block");
                            search.html("<h3>Không có sản phẩm</h3>");
                            $(".container_list").css("display","block");
                        }
                    },
                    error: function () {
        ...
        //    $.ajax({
        //        url: "<?php //echo base_url(); ?>//voxy_hangtralai_nhacc/add_save",
        //        async: false,
        //        type: "POST",
        //        dataType: "json",
